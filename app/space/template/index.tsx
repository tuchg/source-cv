...
import { useMyTemplates } from "@/services/space"
...
function MyTemplates({ onInstall }) {
  ...
  return (
    <div className="grid grid-flow-row grid-cols-4 2xl:grid-cols-3  lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2  gap-5 row-auto pb-4 place-self-center place-items-center place-content-center">
      {data.length > 0 ? (
        // @ts-ignore
        data.map((template) => (
          <Card className="space-y-3 p-4 max-w-[26rem]" key={template.id}>
              ...
              <div className="flex justify-between flex-row items-center">
                ...
                {template.name === currTemplate ? (
                  ...
                  <>
                    ...
                    <Button onClick={() => onDel(template.id)} variant="ghost">
                      <Icons.del size={16} />
                    </Button>
                  </>
                )}
        ...
        <div className="mx-auto w-full h-full col-span-2 text-center">
          暂无数据
        </div>
      )}
    </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="max-w-[20rem] text-xl font-bold">
                  {item.package.name.split("-theme-")[1]}
                </h2>
                <p className="max-w-[20rem]">{item.package.description}</p>
              </div>
              <div className="flex-1"></div>
              <div>
                <p>最近更新：{dayjs(item.package.date).format("YYYY-MM-DD")}</p>
